...
public class DepthOfField : PostEffectsBase
{
	...
	private static readonly int s_curveParams = Shader.PropertyToID("_CurveParams");

	private static readonly int s_offsets = Shader.PropertyToID("_Offsets");

	private static readonly int s_lowRez = Shader.PropertyToID("_LowRez");

	private static readonly int s_fgOverlap = Shader.PropertyToID("_FgOverlap");

	private static readonly Vector4 s_zeroOneFive = new Vector4(0f, 1.5f, 0f, 1.5f);

	private static readonly Vector4 s_oneFiveZero = new Vector4(1.5f, 0f, 0f, 1.5f);
	...
	private void WriteCoc(RenderTexture fromTo, bool fgDilate)
	{
		...
		{
			fromTo.MarkRestoreExpected();
			RenderTexture temporary3 = RenderTexture.GetTemporary(fromTo.descriptor);
			Graphics.Blit(fromTo, temporary3);
			Graphics.Blit(temporary3, fromTo, dofHdrMaterial, 0);
	...
	private void OnRenderImage(RenderTexture source, RenderTexture destination)
	{
		...
		dofHdrMaterial.SetVector(s_curveParams, new Vector4(1f, focalSize, 1f / (1f - aperture) - 1f, focalDistance01));
		...
		else if (blurType == BlurType.DX11 && (bool)dx11bokehMaterial)
		{
			...
			if (highResolution)
			{
				...
				dofHdrMaterial.SetVector(s_offsets, s_zeroOneFive);
				Graphics.Blit(renderTexture3, renderTexture4, dofHdrMaterial, 19);
				dofHdrMaterial.SetVector(s_offsets, s_oneFiveZero);
				...
				if (nearBlur)
				{
					dofHdrMaterial.SetVector(s_offsets, new Vector4(0f, num2, 0f, num2));
					Graphics.Blit(renderTexture4, renderTexture3, dofHdrMaterial, 2);
					dofHdrMaterial.SetVector(s_offsets, new Vector4(num2, 0f, 0f, num2));
				...
				dofHdrMaterial.SetVector(s_offsets, new Vector4(internalBlurWidth, 0f, 0f, internalBlurWidth));
				Graphics.Blit(renderTexture, source, dofHdrMaterial, 5);
				dofHdrMaterial.SetVector(s_offsets, new Vector4(0f, internalBlurWidth, 0f, internalBlurWidth));
			...
			else
			{
				...
				dofHdrMaterial.SetVector(s_offsets, new Vector4(0f, 1.5f, 0f, 1.5f));
				Graphics.Blit(renderTexture3, renderTexture4, dofHdrMaterial, 19);
				dofHdrMaterial.SetVector(s_offsets, new Vector4(1.5f, 0f, 0f, 1.5f));
				Graphics.Blit(renderTexture4, renderTexture3, dofHdrMaterial, 19);
				...
				dofHdrMaterial.SetVector(s_offsets, 1f * (float)source.width / (1f * (float)renderTexture2.width) * internalBlurWidth * Vector4.one);
		...
		else
		{
			...
			if (highResolution)
			{
				dofHdrMaterial.SetVector(s_offsets, new Vector4(0f, internalBlurWidth, 0.025f, internalBlurWidth));
				Graphics.Blit(source, destination, dofHdrMaterial, pass);
			}
			else
			{
				dofHdrMaterial.SetVector(s_offsets, new Vector4(0f, internalBlurWidth, 0.1f, internalBlurWidth));
				Graphics.Blit(source, renderTexture, dofHdrMaterial, 6);
				Graphics.Blit(renderTexture, renderTexture2, dofHdrMaterial, pass);
				dofHdrMaterial.SetTexture(s_lowRez, renderTexture2);
				dofHdrMaterial.SetTexture(s_fgOverlap, null);
				dofHdrMaterial.SetVector(s_offsets, Vector4.one * (1f * (float)source.width / (1f * (float)renderTexture2.width)) * internalBlurWidth);
